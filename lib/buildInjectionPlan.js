...
export function buildInjectionPlan(fieldValues, schema) {
  const plan = {
    ...
    hospital: null,  // { id, searchText }  (account data — deferred)
  ...
  schema.fields.forEach((f) => {
    ...
    if (isEmpty(raw)) { plan.skipped.push({ id: f.id, reason: "empty" }); return; }
    ...
    switch (f.inputType) {
      ...
      case "autocomplete": {
        if (f.id === "procedure") {
          ...
          // hospital ids are account-specific; defer (left for user for now)
          plan.skipped.push({ id: f.id, reason: "hospital needs account data (deferred)" });
        }
        break;
      }
      default: {
