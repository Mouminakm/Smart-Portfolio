...
import { PATIENT_IDENTIFIER_SET } from "./patientFields";

// Fields we must never auto-fill (spec F11: patient identifiers) or that aren't
// real platform fields.
// Patient identifiers — never dictated, parsed, or auto-filled (spec F11).
// Applies to ALL specialties' proformas. The clinician enters these directly
// on eLogbook if required.


// Patient-identifier fields that must never be auto-filled (spec F11) now live
// in lib/patientFields.js — the single source of truth, shared with the
// dictation screen. Imported above as PATIENT_IDENTIFIER_SET.
...
// This is applied to BOTH the dictated text and the schema/procedure names, so
// both sides land on the same spelling before we compare.
function norm(s) {
  return String(s)
    .trim()
    .toLowerCase()
...
// procedure), so matching them is safe. Order matters: exact matches (on the
// name OR an alias) win first, then looser "contains" matches, so a precise
// abbreviation like "ETV" beats an accidental substring hit.
function resolveProcedure(rawValue, procedures) {
  const v = norm(rawValue);
  const list = procedures || [];

  // Does this procedure have an alias that EXACTLY equals the dictated value?
  const aliasExact = (p) =>
    (p.aliases || []).some((a) => norm(a) === v);

  // Does this procedure have an alias the dictated value contains, or vice versa?
  const aliasLoose = (p) =>
    (p.aliases || []).some((a) => {
      const na = norm(a);
  ...
  let hit =
    ...
    // 2. exact alias (e.g. "ETV" -> Endoscopic third ventriculostomy)
    list.find(aliasExact) ||
    // 3. name contains either direction (tolerant of minor differences)
    list.find((p) => norm(p.name).includes(v)) ||
    list.find((p) => v.includes(norm(p.name))) ||
    // 4. alias contains either direction (loosest)
    list.find(aliasLoose);
...
function normaliseDate(raw) {
  ...
  if (parts.length === 3) {
    ...
    // Year is last. The upstream parser always hands us UK order (dd-mm), so the
    // first number is the day and the second the month — no format-guessing.
    // (The old code assumed US mm/dd here and silently swapped every date on or
    // before the 12th, e.g. 05-03 -> 03-05. That was the date-corruption bug.)
    const day = a, month = b, year = c;
...
export function buildInjectionPlan(fieldValues, schema, userHospitals = [], userSpecialty = "") {
  ...
  schema.fields.forEach((f) => {
    ...
    if (PATIENT_IDENTIFIER_SET.has(f.id)) { plan.skipped.push({ id: f.id, reason: "never auto-fill (identifier)" }); return; }
